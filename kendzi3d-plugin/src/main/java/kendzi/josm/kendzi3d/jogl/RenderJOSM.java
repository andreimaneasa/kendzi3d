...
import kendzi.josm.kendzi3d.jogl.selection.MyDropTargetListener;
...
public class RenderJOSM implements DataSetListenerAdapter.Listener{
	...
	public RenderJOSM() {
		...
		DatasetEventManager.getInstance().addDatasetListener(
				new DataSetListenerAdapter(this), FireMode.IMMEDIATELY);
	...
	kendzi.jogl.model.geometry.Model createErrorModel() {
		...
		Material material = new Material(new AmbientDiffuseComponent(Color.RED,
				Color.RED));
	...
	private void setupPerspective3D(EastNorth center) {
		...
		this.perspective = new Perspective3D(scale, center.getX(),
				center.getY());
	}
	...
	private EastNorth centerFromDataSet(DataSet dataset, Projection proj) {
		...
		if (dataset.dataSources.size() > 0) {
			...
			for (org.openstreetmap.josm.data.DataSource source : dataset.dataSources) {
		...
		return proj.latlon2eastNorth(new LatLon((maxY + minY) / 2d,
				(maxX + minX) / 2d));
	}
