...
// 8 iulie creata ca sa pot avea BuildingModel
//	public static BuildingModel parseBuildingWayPrimitives(Way way, Perspective perspective) {
//		BuildingModel bm = new BuildingModel();
//		bm.setParts(parseBuildingPrimitive(way, perspective));
//
//		return bm;
//	}
...
//		bp.setFacadeColor(BuildingAttributeParser.parseRoofColor(node)); //////add by me Andrei in 30 July nu are efect direct la parsarea pt culoare
...
// 8 iulie generate de mine pt ca sa pot lua primitivele ca sa fac select 
// am creat si getter si setter pt BuildingPart() ca sa pot seta osm primitive 
	
	public static BuildingPart parseBuildingPrimitiveAttributes(OsmPrimitive pOsmPrimitive) {
		BuildingPart bp = new BuildingPart();
		bp.setOsmPrimitive(pOsmPrimitive);
		
		return bp;
	}
// 	metoda creata ca sa pot sa am o lista de  BuildingPart mai sus e apelata
//	private static List<BuildingPart> parseBuildingPrimitive (Way primitive, Perspective pPerspective) {
//		
//		OsmPrimitive p = primitive;
//		BuildingPart bp = parseBuildingPrimitiveAttributes(p); 
//		
//		bp.setWall(parseWall((Way) p, pPerspective));
//		bp.setInlineWalls(null);
//
